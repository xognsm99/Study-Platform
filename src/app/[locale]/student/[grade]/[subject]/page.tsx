...
export default async function SubjectPage({
  ...
  const { locale, grade, subject } = await params;
      ...
      <div className="mx-auto max-w-4xl p-6 text-gray-900">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
          {ENGLISH_CATEGORIES.map((c) => (
            <Link
              key={c.key}
              href={`/${locale}/student/${encodeURIComponent(decodedGrade)}/english/${c.key}`}
        ...
        <div className="mt-6">
          <Link
            href={`/${locale}/student/${encodeURIComponent(decodedGrade)}`}
